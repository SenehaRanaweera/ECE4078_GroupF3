...
class Robot:
    ...
    # --------------------------
    #Tell the EKF how the next pose changes with respect to the current state [x,y,θ] for the given control (wheel speeds, dt).
    def derivative_drive(self, drive_meas):
        ...
        # we used the matrix jacobian
        if ang_vel == 0:
            DFx[0,2] += -np.sin(self.state[2]) * lin_vel * dt
            DFx[1,2] += np.cos(self.state[2]) * lin_vel * dt
        else:
            th = self.state[2]
            DFx[0,2] += lin_vel / ang_vel * (np.cos(th+dt*ang_vel) - np.cos(th))
            # DFx[1,2] += lin_vel / ang_vel * (np.sin(th+dt*ang_vel) - np.sin(th)) # swap sines
            DFx[1,2] += lin_vel / ang_vel * (np.sin(th) - np.sin(th+dt*ang_vel))
    ...
    def covariance_drive(self, drive_meas): 
        ...
        if ang_vel == 0:
            ...
            Jac2[0,0] = (-np.sin(th)/ang_vel) + (np.sin(th2)/ang_vel)
            Jac2[0,1] = (lin_vel/ang_vel)*dt*np.cos(th2)+(lin_vel/(ang_vel**2))*(np.sin(th)-np.sin(th2))

            Jac2[1,0] = (np.cos(th)/ang_vel) - (np.cos(th2)/ang_vel)
            Jac2[1,1] = (lin_vel/ang_vel)*dt*np.sin(th2)+(lin_vel/(ang_vel**2))*(-np.cos(th)+np.cos(th2))

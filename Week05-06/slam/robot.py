...
class Robot:
    ...
    # --------------------------
      ##Tell the EKF how the next pose changes with respect to the current state [x,y,theta] for the given control (wheel speeds, dt). 
      #It is computing the Jacobian of the motion model w.r.t. the robot state x,y,theta
    def derivative_drive(self, drive_meas): 
        ...
        th = self.state[2] #current heading theta
    ...
    def covariance_drive(self, drive_meas): #How uncertainty in controls/wheels injects new noise?
        ...
        if abs(ang_vel) < eps:
            ...
            # General differential-drive motion
            S = np.sin(th2) - np.sin(th)
            ...
            Jac2[2,0] = 0.0
            # ∂x/∂w, ∂y/∂w
            Jac2[0,1] = lin_vel * (ang_vel * np.cos(th2) * dt - S) / (ang_vel**2)
            Jac2[1,1] = lin_vel * (ang_vel * np.sin(th2) * dt + C) / (ang_vel**2)
            Jac2[2,1] = dt

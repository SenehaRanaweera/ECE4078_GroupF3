...
class EKF:
    ...
    def predict(self, raw_drive_meas):
        ...
        Q = self.predict_covariance(raw_drive_meas) #Build the process-noise covariance Q, capturing uncertainty in motion (e.g., wheel slip, encoder noise, timing).
    ...
    def update(self, measurements):
        ...
        #Extract landmark IDs from the incoming measurements and map each ID to your internal landmark index via self.taglist
        ...
        # predicted measurements for the same landmarks and reshape them into (2m×1).
        z_hat = self.robot.measure(self.markers, idx_list)
        z_hat = z_hat.reshape((-1,1),order="F") #(Fortran/column-major)
        H = self.robot.derivative_measure(self.markers, idx_list)

        x = self.get_state_vector()

        # TODO: add your codes here to compute the updated x
        # Innovation what you observed minus what you expected. This is the new information that will correct the prediction.
        y = z - z_hat
        # Innovation covariance, Quantifies how uncertain the innovation is, considering both prior and measurement noise.
        S = H @ self.P @ H.T + R  
        # Kalman gain, how much to trust measurement vs. prior.
        K = self.P @ H.T @ np.linalg.inv(S)
        # State update
        x = x + K @ y
        # Covariance update (simple form), Reduce uncertainty in directions measured.
    ...
    def predict_covariance(self, raw_drive_meas): #build the process-noise covariance matrix Q used in the EKF predict step:
        n = self.number_landmarks()*2 + 3
        Q = np.zeros((n,n))
